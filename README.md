...
- Support the development of the upstream image with a [donation](http://www.damagehead.com/donate/)
...
```bash
docker pull leftathome/gitlab-ee:8.12.9
```
...
```bash
docker build -t leftathome/gitlab-ee github.com/leftathome/gitlab-ee
```
...
```bash
wget https://raw.githubusercontent.com/leftathome/docker-gitlab/master/docker-compose.yml
```
...
docker run --name gitlab -d [OPTIONS] \
    leftathome/gitlab-ee:8.12.9 app:sanitize
```
...
| `SKIP_SIDEKIQ` | Don't run Sidekiq on this host. |
...
| `OAUTH_GITHUB_SCOPE` | GitHub OAuth scope.  Defaults to `user:email` |
| `OAUTH_GITHUB_URL` | GitHub URL.  Override for Enterprise installs.  Defaults to `https://github.com` |
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.12.9 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.12.9 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.12.9 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.12.9 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.12.9 app:rake gitlab:import:repos
```
