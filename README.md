...
I've made some minor adjustments to the examples in this documentation
for everyone who's turned their brain off and is just copying and pasting
random stuff in from the Internet in the hopes that something works.
...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/leftathome/gitlab-ee) and is the recommended method of installation.

> **Note**: Builds are also available on [Quay.io](https://quay.io/repository/sameersbn/gitlab)

```bash
docker pull leftathome/gitlab-ee:8.7.2
```

You can also pull the `latest` tag which is built from the repository *HEAD*

```bash
docker pull leftathome/gitlab-ee:latest
```

Alternatively you can build the image locally.

```bash
docker build -t leftathome/gitlab-ee github.com/sameersbn/docker-gitlab
```
...
docker run --name gitlab -d [OPTIONS] \
    leftathome/gitlab-ee:8.7.2 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.7.2 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.7.2 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.7.2 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.7.2 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:8.7.2 app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `leftathome/gitlab-ee:8.6.6` from `leftathome/gitlab-ee:7.x.x` can cause issues. It is therefore required that you first upgrade to `leftathome/gitlab-ee:8.0.5-1` before upgrading to `leftathome/gitlab-ee:8.1.0` or higher.
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    leftathome/gitlab-ee:x.x.x app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -d [OPTIONS] leftathome/gitlab-ee:8.7.2
```
